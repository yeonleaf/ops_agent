#!/usr/bin/env python3
"""
RAG 시스템 정확도 평가 스크립트 (티켓 중복 제거 버전)

중복 제거 전략 비교:
- max_score: 최고 점수 청크만 선택
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/rag_evaluation_dedup.log'),
...
class RAGEvaluator:
        ...
        try:
            ...
            for i, result in enumerate(search_results[:5], start=1):
                ...
                top_results.append({
                    ...
                    'aggregated_chunks': metadata.get('aggregated_chunks', 1)
                })
    ...
    def print_metrics(self, metrics: Dict[str, float], title: str = "RAG 시스템 평가 결과"):
        """메트릭 출력"""
        print("\n" + "="*60)
        print(f"📊 {title}")
        print("="*60)
...
def compare_strategies():
    ...
    try:
        ...
        strategies = ['max_score', 'all_scores', 'first']
        all_results = {}

        for strategy in strategies:
            logger.info(f"\n{'#'*80}")
            logger.info(f"# 중복 제거 전략: {strategy}")
            logger.info(f"{'#'*80}\n")
            ...
            output_path = f"rag_evaluation_dedup_{strategy}_{timestamp}.json"
            config_info = {
                'deduplication_strategy': strategy,
                'deduplicate_tickets': True,
                'rrf_k': rrf_config.rrf_k,
                'enable_bm25': rrf_config.enable_bm25
            }
