...
class IntegratedMailClassifier:
    ...
    def should_create_ticket(self, email_data: Dict[str, Any], user_query: str = "", few_shot_examples: Dict[str, List[Dict[str, Any]]] = None) -> Tuple[TicketCreationStatus, str, Dict[str, Any]]:
        ...
        try:
            ...
            system_prompt = f"""
            당신은 비용 효율성을 최우선으로 생각하는 **깐깐한 수석 매니저**입니다.
            당신의 유일한 임무는 **불필요한 Jira 티켓 생성을 원천 차단**하여 팀의 리소스를 보호하는 것입니다.

            **기본 원칙: 모든 메일은 '티켓 생성 불필요'에서 시작합니다.** 아래의 모든 엄격한 기준을 통과해야만 예외적으로 티켓 생성을 승인할 수 있습니다.
            '의심스러우면 거절' 원칙을 항상 명심하세요.

            {few_shot_prompt}

            ---
            **[사고 과정]**
            새로운 이메일이 주어지면, 반드시 아래의 사고 과정에 따라 단계별로 분석을 수행하고 최종적으로 JSON을 출력하세요.

            1.  **[이메일 핵심 요약]:** 이메일의 발신 목적과 핵심 요청 사항을 한 문장으로 요약합니다.
            2.  **[판단 기준 체크리스트]:** 아래 5가지 핵심 기준을 하나씩 순서대로 평가하고, 각각 `Yes` 또는 `No`로 명시적인 판단을 내립니다.
                - **A. 명시적 요청 존재 여부:** 구체적인 행동(작업, 회신, 검토, 확인, 처리 등)을 직접적으로 요청하는 문장이 있는가? 단순한 정보 공유는 No (Yes/No)
                - **B. 내부 업무 맥락 포함 여부:** 우리 서비스, 프로젝트, 내부 시스템과 직접 관련된 내용인가? 외부 서비스 업데이트나 일반 뉴스는 No (Yes/No)
                - **C. 추적 관리 필요성:** 해당 요청이 완료되기까지 과정을 추적하고 관리해야 할 필요가 있는가? 자동 알림이나 일회성 정보는 No (Yes/No)
                - **D. 절대 생성 금지 목록 제외 여부:** 아래 '절대 생성 금지 목록'에 해당하지 않는가? (Yes/No)
                - **E. 업무 긴급성 존재 여부:** 이 메일이 시급히 처리되어야 할 업무적 가치가 있는가? 뉴스레터, 광고, 개인 메시지는 No (Yes/No)
            3.  **[최종 결론]:** 위 체크리스트의 모든 항목(A, B, C, D, E)이 `Yes`인 경우에만 티켓 생성을 승인합니다. 하나라도 `No`가 있다면 즉시 거절합니다.
            ---

            ❌ **절대 생성 금지 목록 (강력한 제외 기준):**
            - 개인적인 안부, 인사, 잡담 (예: "💬 Mingu Kang", 개인 메시지)
            - 단순 정보 공유 (뉴스레터, 일반 공지, 소식지, "요즘IT", "Issue 295" 등)
            - 광고/마케팅/프로모션 메일 (제목에 (광고), 💰, 📢, [광고] 포함)
            - 자동 알림 및 영수증 (로그인 알림, 결제 영수증, 시스템 알림, "최근 검색에 표시됨", "Google Play 주문 영수증")
            - 구독 서비스 및 교육 관련 메일 (블로그, 웨비나, 온라인 강의, "[매일메일]")
            - 제품 업데이트 및 기능 소개 ("Craft – Liquid Glass 업데이트", "GitHub Universe")
            - 개인정보 관련 자동 통지 ("[구글] 개인정보 이용내역 통지")
            - 슬라이드, 프레젠테이션 관련 광고성 메일
            - HTML/CSS 코드가 주요 내용인 메일
            - 해외 SNS나 개인 블로그 형태의 메일 ("Foreigner begging", "Had the weirdest thing")

            ⚠️ **주의:** 위 체크리스트 **A, B, C, D, E 모두를 통과하지 못하면**, '의심스러우면 거절' 원칙에 따라 즉시 `should_create_ticket: false`로 결정해야 합니다.
            ...
            {{
                "reasoning_steps": {{
                    "summary": "...",
                    "checklist": {{
                        "A_explicit_request": "Yes|No",
                        "B_internal_context": "Yes|No",
                        "C_tracking_needed": "Yes|No",
                        "D_exclusion_list_check": "Yes|No",
                        "E_business_urgency": "Yes|No"
                    }},
                    "final_conclusion": "..."
                }},
...
사용자가 티켓/업무 관련 요청을 했고, 이 메일이 업무 관련이라면 티켓을 생성해야 합니다."""
            ...
            try:
                ...
                # reasoning_steps가 있으면 상세한 분석 결과 사용
                reasoning_steps = lm_result.get('reasoning_steps', {})
                if reasoning_steps:
                    reasoning = reasoning_steps.get('final_conclusion', '')
                    checklist = reasoning_steps.get('checklist', {})
                    
                    # 체크리스트 결과 로깅
                    self._safe_log(f"   📋 체크리스트 결과:", "info")
                    self._safe_log(f"      - A. 명시적 요청: {checklist.get('A_explicit_request', 'N/A')}", "info")
                    self._safe_log(f"      - B. 내부 업무 맥락: {checklist.get('B_internal_context', 'N/A')}", "info")
                    self._safe_log(f"      - C. 추적 관리 필요: {checklist.get('C_tracking_needed', 'N/A')}", "info")
                    self._safe_log(f"      - D. 제외 목록 체크: {checklist.get('D_exclusion_list_check', 'N/A')}", "info")
                    self._safe_log(f"      - E. 업무 긴급성: {checklist.get('E_business_urgency', 'N/A')}", "info")
                else:
                    # 기존 형식 fallback
    ...
    def _should_create_ticket_fallback(self, email_data: Dict[str, Any], user_query: str = "") -> Tuple[TicketCreationStatus, str, Dict[str, Any]]:
        ...
        # 3단계: 사용자가 명시적 의도를 보였어도, 메일 내용이 실제 업무와 관련이 있는지 엄격히 확인
        email_has_keywords = self._check_ticket_keywords_in_email(email_data)
        
        # 업무 관련 키워드가 없으면 티켓 생성하지 않음 (더 엄격한 기준)
        if not email_has_keywords:
            return TicketCreationStatus.NO_TICKET_NEEDED, "기본 규칙: 명시적 의도 있으나 업무 관련 키워드 없음", {
                'fallback_reason': 'LM 사용 불가 + 명시적 의도 + 업무 키워드 없음',
                'explicit_intent': True,
                'email_keywords': email_has_keywords,
                'spam_score': spam_score,
                'strict_rule': '업무 키워드 필수'
            }
        
        # 4단계: 광고가 아니고 업무 키워드가 있으면 티켓 생성
        return TicketCreationStatus.SHOULD_CREATE, "기본 규칙: 명시적 의도 + 업무 키워드 + 광고 아님", {
            'fallback_reason': 'LM 사용 불가 + 명시적 의도 + 업무 키워드 + 광고 아님',
            'explicit_intent': True,
            'email_keywords': email_has_keywords,
            'spam_score': spam_score,
            'strict_rule': '업무 키워드 필수 + 광고 제외'
        }
