#!/usr/bin/env python3
"""
ChromaDB에 한국어 특화 임베딩 모델(ko-sroberta-multitask)을 설정하는 스크립트
"""
...
class KoreanEmbeddingFunction:
    ...
    def __init__(self, model_name: str = "jhgan/ko-sroberta-multitask"):
        ...
        logger.info(f"🔧 한국어 임베딩 모델 로딩 중: {model_name}")
        try:
    ...
    def __call__(self, input):
        ...
        try:
            ...
            logger.info(f"✅ {len(input)}개 텍스트 임베딩 완료 (차원: {len(normalized_embeddings[0]) if normalized_embeddings else 0}, L2 정규화 적용)")
            return normalized_embeddings
...
def setup_korean_embedding():
    """ChromaDB에 한국어 특화 임베딩 함수 설정"""
    logger.info("🔧 ChromaDB에 한국어 특화 임베딩 함수 설정 중...")
    ...
    # 한국어 임베딩 함수 초기화
    try:
        korean_embedding_function = KoreanEmbeddingFunction()
    except Exception as e:
    ...
    for collection_info in collections:
        ...
        try:
            ...
            new_collection = client.create_collection(
                ...
                metadata={
                    "description": f"한국어 특화 임베딩 사용 - {collection_name}",
            ...
            logger.info(f"   ✅ 한국어 임베딩 함수 설정 완료: {collection_name}")
...
def test_korean_embedding():
    ...
    try:
        ...
        # 임베딩 생성
        embeddings = korean_embedding_function(test_texts)
        ...
        logger.error(f"❌ 한국어 임베딩 테스트 실패: {e}")
        return False

if __name__ == "__main__":
    # 테스트 먼저 실행
    if test_korean_embedding():
        # 테스트 성공 시 설정 실행
        setup_korean_embedding()
    else:
